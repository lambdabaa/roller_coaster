...
public class RollerCoaster implements GLEventListener {
  ...
  static Parser parser;     // TODO(garethaye): Figure out why this is static
  static Parser parser2;    // TODO(garethaye): Figure out why this is static
  ...
  TextRenderer renderer = new TextRenderer(new Font("SansSerif", Font.PLAIN, 16));
    ...
    for (int i = 0; i < num_rails; i++) {
      centerPos[i] = 
          new Point3(
              0.3 * i / 20.0 * Math.sin(dtheta * i),
              3 * Math.sin(i / 20.0) + 3,
              0.3 * i / 20.0 * Math.cos(dtheta * i));
    }
    ...
    if (groundTexture == null) {
      LOGGER.info("Didn't get a ground texture...");      // TODO(garethaye): ???
      System.exit(0);
    }
    ...
    private void drawCart(GL2 gl) {
    	...
    	gl.glPushMatrix();
    	gl.glTranslated(
    	    centerPos[(pos + 5) % num_rails].getX(),
    	    centerPos[(pos + 5) % num_rails].getY() + 0.05f,
    	    centerPos[(pos + 5) % num_rails].getZ());
        ...
        gl.glColor3i(255, 255, 255);
    	  gl.glNormal3f(
    	      parser2.normals[parser2.faces[i].a_n].nx,
    	      parser2.normals[parser2.faces[i].a_n].ny,
    	      parser2.normals[parser2.faces[i].a_n].nz);
        gl.glTexCoord2f(0.0f, 0.0f); 
        gl.glVertex3f(
            parser2.vertices[parser2.faces[i].a].x,
            parser2.vertices[parser2.faces[i].a].y,
            parser2.vertices[parser2.faces[i].a].z); 
	      gl.glNormal3f(
	          parser2.normals[parser2.faces[i].b_n].nx,
	          parser2.normals[parser2.faces[i].b_n].ny,
	          parser2.normals[parser2.faces[i].b_n].nz);
        gl.glTexCoord2f(1.0f, 0.0f); 
        gl.glVertex3f(
            parser2.vertices[parser2.faces[i].b].x,
            parser2.vertices[parser2.faces[i].b].y,
            parser2.vertices[parser2.faces[i].b].z); 
	      gl.glNormal3f(
	          parser2.normals[parser2.faces[i].c_n].nx,
	          parser2.normals[parser2.faces[i].c_n].ny,
	          parser2.normals[parser2.faces[i].c_n].nz);
        gl.glTexCoord2f(1.0f, 1.0f); 
        gl.glVertex3f(
            parser2.vertices[parser2.faces[i].c].x,
            parser2.vertices[parser2.faces[i].c].y,
            parser2.vertices[parser2.faces[i].c].z); 
	      gl.glNormal3f(
	          parser2.normals[parser2.faces[i].d_n].nx,
	          parser2.normals[parser2.faces[i].d_n].ny,
	          parser2.normals[parser2.faces[i].d_n].nz);
        gl.glTexCoord2f(0.0f, 1.0f); 
        gl.glVertex3f(
            parser2.vertices[parser2.faces[i].d].x,
            parser2.vertices[parser2.faces[i].d].y,
            parser2.vertices[parser2.faces[i].d].z); 
    ...
    private void drawGround(GL2 gl) {
      ...
      gl.glBegin(GL2.GL_QUADS);
      gl.glTexCoord2f(0.0f, 0.0f);
      gl.glVertex3f(20.0f, 0.0f,  -20.0f);  // Bottom Left Of The Texture and Quad
      gl.glTexCoord2f(10.0f, 0.0f);
      gl.glVertex3f(-20.0f, 0.0f,  -20.0f);  // Bottom Right Of The Texture and Quad
      gl.glTexCoord2f(10.0f, 10.0f);
      gl.glVertex3f(-20.0f,  0.0f,  20.0f);  // Top Right Of The Texture and Quad
      gl.glTexCoord2f(0.0f, 10.0f);
      gl.glVertex3f(20.0f,  0.0f,  20.0f);  // Top Left Of The Texture and Quad
      gl.glEnd();
    }
    ...
    private void drawSkybox(GL2 gl) {
    	...
    	gl.glBegin(GL2.GL_QUADS);
    	gl.glTexCoord2f(0.0f, 0.0f);
    	gl.glVertex3f(-20f, 20f,  -20f);  // Bottom Left Of The Texture and Quad
    	gl.glTexCoord2f(1.0f, 0.0f);
    	gl.glVertex3f(20f, 20f,  -20f);  // Bottom Right Of The Texture and Quad
    	gl.glTexCoord2f(1.0f, 1.0f);
    	gl.glVertex3f(20f, 20f,  20f);  // Top Right Of The Texture and Quad
    	gl.glTexCoord2f(0.0f, 1.0f);
    	gl.glVertex3f(-20f, 20f, 20f);  // Top Left Of The Texture and Quad
	    gl.glEnd();
	    ...
	    gl.glBegin(GL2.GL_QUADS);
	    gl.glTexCoord2f(0.0f, 0.0f);
	    gl.glVertex3f(-20f, 0f,  20f);  // Bottom Left Of The Texture and Quad
	    gl.glTexCoord2f(1.0f, 0.0f);
	    gl.glVertex3f(-20f, 0f,  -20f);  // Bottom Right Of The Texture and Quad
	    gl.glTexCoord2f(1.0f, 1.0f);
	    gl.glVertex3f(-20f, 20f,  -20f);  // Top Right Of The Texture and Quad
	    gl.glTexCoord2f(0.0f, 1.0f);
	    gl.glVertex3f(-20f, 20f,  20f);  // Top Left Of The Texture and Quad
      ...
      try {
        texture =
            TextureIO.newTexture(
                new File(
                    this.getClass().getClassLoader().getResource(string).getFile()),
                true);
      } catch (IOException e) {
        e.printStackTrace();
      }
  ...
  private void update() {
    ...
    st = String.format("%s%s person view", st, firstPerson ? "first" : "third");
